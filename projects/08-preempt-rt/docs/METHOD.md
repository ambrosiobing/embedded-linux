...
| The same matrix on the Pi 3B as well as the Pi 4 | a slower core, 1 GB, Ethernet behind the USB hub: a harder real-time target under the same kernel | both images are built and archived, so this is a second set of runs rather than a second build |
...
|---|---|---|
| 60 s per run | 30000 rising edges makes a 99.9th percentile mean something; longer and a bare Pi under `stress-ng` starts throttling mid-run | a heatsink, and then 300 s runs for the rows that matter |
