...
| 05 | [An IIO driver for the ADXL345 written from scratch](projects/05-iio-adxl345) | Raspberry Pi 3 | Kernel driver model, regmap, threaded IRQ, IIO events | **Written, not yet built**: the driver, its recipe, the kernel fragment, the image and the build configuration, with 43 assertions that need no board and no kernel tree |
...
| 20 | [OP-TEE on the Pi 3: a trusted application for key storage](projects/20-optee-keystore) | Raspberry Pi 3 | TrustZone, OP-TEE OS, TEE Client API, secure storage | **Normal world built, no secure world yet**: the image exists, 58 MB in 43 minutes at `4f84a65`, with the trusted application linked and signed for the first time. Getting there took four builds, each failing one step further along: no provider for `python3-gpiod`, the TA unable to find libgcc, the CLI unable to find its own library, and a `MAX_INPUT` that collided with a kernel header. Every one of those had passed "software complete" because nothing had ever compiled it. `armstub8.bin` comes from a separate OP-TEE checkout that has not been built, so a board booted from this image finds no TEE, which is the expected state and not a result. No board work yet |
