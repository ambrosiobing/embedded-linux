...
| `./go tee-mod` | `kas/bench-tee-modular.yml` | The same image with the OP-TEE driver built as a module and `kernel-module-optee` installed, so a driver that hangs at probe can be loaded from a login instead of at boot. A debugging image, to be deleted when the hang is understood |
