...
| 02 | [NanoPi NEO Air on mainline: U-Boot, kernel, device tree, eMMC](projects/02-neo-air-mainline) | NanoPi NEO Air | Board bring-up, bootloader, sunxi mainline | **Booting from eMMC**: U-Boot v2025.10, a 6.12 kernel with the board device tree and brcmfmac as a module, and a Debian bookworm armhf root filesystem carrying the modules and the AP6212 firmware, with 69 assertions covering the two tools that can destroy something. Criteria 1 to 4 met on hardware: U-Boot sees both media, the card boots to a login, Wi-Fi associates and takes a DHCP lease, and the board boots its own eMMC with no card in the slot. Boot logs for both media are in docs/. Criteria 5 and 6, idempotence and FEL recovery, are next |
| 03 | [Boot time and energy per boot, measured](projects/03-boot-energy) | NanoPi NEO Air + PPK2 | Boot-time optimisation, systemd-analyze, power | **Software complete**: design, bring-up order, the two host programs with 49 assertions against synthetic traces, the loop driver, the marker unit and device-tree node, and the U-Boot and kernel fragments. **Blocked on a system to boot**: Project 2 builds it and has not produced an image yet, so every measured cell is blank |
