...
| 02 | [NanoPi NEO Air on mainline: U-Boot, kernel, device tree, eMMC](projects/02-neo-air-mainline) | NanoPi NEO Air | Board bring-up, bootloader, sunxi mainline | **Building**: U-Boot and kernel build scripts, the rootfs and its overlay, the FEL, SD card and eMMC tools, with 61 assertions covering the two that can destroy something. all three artefacts build: U-Boot v2025.10, a 6.12 kernel with the board device tree and brcmfmac as a module, and a 393 MB Debian bookworm armhf root filesystem carrying the modules and the AP6212 firmware. No board has been powered on |
