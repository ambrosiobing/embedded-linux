...
| `libsystemd-dev` | Project 12's daemon is written against `sd-bus` and `sd-event`. The vtable that declares its D-Bus interface is a table of macros, so a wrong signature or a misplaced flag is a compile error rather than a run-time surprise, and this is the only check short of a board that can see it |
| `libcbor-dev` | The same daemon decodes the hub's payloads with libcbor. Encoding is done by hand in `proto.c` because the shapes are fixed; decoding is not, because the bytes come off a wire and belong in a parser somebody else has fuzzed |
